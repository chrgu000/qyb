...
public class QybController extends BaseController {
  ...
  public String selectStatus(String userId, HttpServletResponse response) {
    ApplyCash applyCash = applyService.selectStatus(userId);
    if (applyCash == null) {
      return renderString(response, BaseResponse.success("0"));
    } else {
      return renderString(response, BaseResponse.success("1"));
    }
  }
